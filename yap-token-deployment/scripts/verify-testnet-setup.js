...
async function verifyTestnetSetup() {
  ...
  try {
    ...
    if (network.chainId.toString() === '1115') {
      console.log('✅ Chain ID 正确 (1115 - Core 测试网)');
    } else {
      console.log('❌ Chain ID 不正确，应该是 1115');
      console.log('💡 请确保 MetaMask 已切换到 Core 测试网');
    }
    ...
    const checks = [
      { name: '网络连接', passed: network.chainId.toString() === '1115' },
